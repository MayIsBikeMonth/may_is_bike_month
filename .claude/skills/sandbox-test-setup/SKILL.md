...
description: >-
  May is Bike Month Ruby + RSpec environment setup. Two environments:
  **(A) local macOS Conductor workspace** (`/Users/…/conductor/workspaces/…`) —
  ...
  (4.0.9)`. Fix is a PATH prefix, not a reinstall.
  **(B) Claude Code's Linux web sandbox** (`/home/user/may_is_bike_month`) —
...
fails with `Could not find 'bundler' (4.0.9)`. **The Ruby is
...
`BUNDLED WITH 4.0.9`. No prebuilt 4.0.2 binary is reachable
...
cd /home/user/may_is_bike_month
...
warning). Create the `rails` superuser + test DB once per machine.
...
sudo -u postgres psql -c "CREATE DATABASE may_is_bike_month_test OWNER rails;"
...
The importmap pins four modules (luxon, @bikeindex/time-localizer,
@floating-ui/dom, @honeybadger-io/js) from `cdn.jsdelivr.net` (403'd)
...
```bash
mkdir -p /tmp/cdn /tmp/cdn/luxon /tmp/cdn/bikeindex-time-localizer \
         /tmp/cdn/floating-ui-dom /tmp/cdn/honeybadger-io-js
curl -sL "https://registry.npmjs.org/luxon/-/luxon-3.7.1.tgz" \
  | tar -xz -C /tmp/cdn/luxon --strip-components=1
...
ln -sf /tmp/cdn/luxon /tmp/cdn/serve/npm/luxon@3.7.1
...
The `--host-resolver-rules` argument (in the Chrome override above)
routes `cdn.jsdelivr.net` → this local server, and
`--ignore-certificate-errors` trusts the self-signed cert.
