...
**Body structure.** Open with the bullets directly — **do not** add a `## Summary` heading above them, the bullets are the summary. Use `## Test plan` (and later `## Screenshots`) as the only top-level headings. **Do not** append a `🤖 Generated with Claude Code` trailer or any other generated-by/co-author line in the PR body — leave attribution to the commit's `Co-Authored-By` trailer.

**Test plan content.** The Test plan section is a manual-verification checklist for the reviewer (or the author re-checking before merge), not a record of automated coverage. **Never** include "run `bundle exec rspec ...`", "run the test suite", "lint passes", "CI green", or anything else CI already does — those are not the reviewer's job. Each bullet should describe a concrete thing to *click, observe, or compare in the running app* that the diff is supposed to affect: a specific URL to visit, an interaction to perform, a state to confirm, an edge case (legacy data, empty state, mobile width, settings toggle) to spot-check. If you cannot think of anything manually checkable, omit the Test plan section entirely rather than padding it with CI-shaped bullets.
